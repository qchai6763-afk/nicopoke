...
function searchDebug(parsedCode, found, extra) {
  return Object.assign(
    {
      store: STORE_NAME,
      siteID: process.env.SITE_ID || process.env.NETLIFY_SITE_ID || null,
      context: process.env.CONTEXT || null,
      received: parsedCode && parsedCode.raw,
      normalized: parsedCode && parsedCode.code,
      searchKey: found && found.key,
      searchKeys: (found && found.tried) || (found && found.key ? [found.key] : []),
      hit: Boolean(found && found.data),
      via: found && found.via ? found.via : null,
      listedKeys: (found && found.listedKeys) || [],
      listError: found && found.listError,
    },
    extra || {}
  );
}
...
async function loadGroup(store, code) {
  ...
  const tried = [key];
  console.log("[group] blobs search", { store: STORE_NAME, key });
  const direct = await readBlob(store, key);
  if (direct) return { key, data: direct, tried, via: "direct" };
  return { key, data: null, tried, listedKeys: [] };
}

async function saveGroup(store, code, data) {
  const key = blobKey(code);
  data.group.code = key;
  console.log("[group] writing", { store: STORE_NAME, key });
  await store.setJSON(key, data);
  const check = await store.get(key, { type: "json" });
  if (!parseBlob(check)) {
    throw new Error("setJSON completed but get returned empty");
  }
  console.log("[group] write ok", { store: STORE_NAME, key });
  return key;
}
...
export default async (request) => {
  try {
    ...
    try {
      ...
      console.error("[group] getStore failed", err);
    ...
    console.log("[group] code", {
      ...
      before: parsedCode.raw,
      after: parsedCode.code,
      store: STORE_NAME,
    });
    const code = parsedCode.code;
    if (!/^[A-Z2-9]{4,8}$/.test(code)) {
      return fail(400, "BAD_CODE", {
        debug: searchDebug(parsedCode, { key: parsedCode.code, tried: [], data: null }),
      });
    }
    ...
    if (request.method === "GET") {
      ...
      if (!found.data) {
        return fail(404, "NOT_FOUND", {
          code,
          key: found.key,
          debug: searchDebug(parsedCode, found),
        });
      }
    ...
    if (isWrite) {
      if (!payload || !payload.group) {
        return fail(400, "NEED_GROUP", {
          debug: searchDebug(parsedCode, { key: code, tried: [code], data: null }),
        });
      }
      ...
      try {
        ...
        const saveKey = await saveGroup(store, code, merged);
        return json(200, {
          ok: true,
          stored: true,
          errorCode: null,
          store: STORE_NAME,
          group: { id: merged.group.id, code: saveKey, key: saveKey },
        });
      } catch (err) {
    ...
    return fail(405, "METHOD", {
      debug: searchDebug(parsedCode, { key: code, tried: [], data: null }),
    });
  } catch (err) {
    console.error("[group] unhandled", err);
