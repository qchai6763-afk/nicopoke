...
const DIFF_SPECS = {
  ...
  order: {
    easy: { max: 5, total: 3, noClock: true, limitMs: 0, perRound: false },
    normal: { max: 15, total: 3, noClock: true, limitMs: 0, perRound: false },
    hard: { max: 10, total: 3, noClock: false, limitMs: 15000, perRound: false },
  },
  mood: {
...
const DIFF_HELP = {
  space: {
    ...
    normal: "見本は10回、3×3マス。1問5秒です。",
    hard: "見本は15回、4×4マス。全問通しで30秒です。",
  },
  order: {
...
const BRAIN_INTRO = {
  ...
  mood: {
    ...
    steps: [
      "大きな顔を、ゆっくり見てください。",
      "簡単・普通は、8つの気持ちから選びます。",
      "難しいは、擬音語・擬態語20語から、細かい心の動きを読みます。押した瞬間に正解・はずれが出ます。",
    ],
  },
  nazo: {
    title: "ナゾナゾひらめき",
...
function renderOrderFail() {
  ...
  app.innerHTML = chrome(
    `
      ...
      <div class="play-result">
        ...
        <p class="help">3回通しで ${game.limitMs / 1000} 秒に間に合いませんでした。${game.round} / ${
          game.total
        } 回まで進みました。</p>
        <button class="primary" type="button" data-order-retry>はじめから</button>
  ...
  bindTop();
  app.querySelector("[data-order-retry]")?.addEventListener("click", () => {
...
function renderOrder() {
  ...
  if (game.failed) return renderOrderFail();
...
function ensureMood() {
  if (!window.__mood) {
    ...
    window.__mood = { i: 0, qs, score: 0, onomato: spec.onomato, lock: false, lastJudge: null };
  }
...
function renderMood() {
  ...
  const judge =
    game.lastJudge == null
      ? ""
      : `<p class="mood-judge ${game.lastJudge ? "ok" : "ng"}">${game.lastJudge ? "正解！" : "はずれ"}</p>`;
  app.innerHTML = chrome(
    `
      <a class="back-link" href="#/brain">← 脳トレ一覧</a>
